            ...
            public class DalObject
            {
                ...
                public static string Distance(int ID, double lonP, double latP)
                {
                    if(ID > 9999)//if its a customer
                        ...
                        DataSource.stationList.ForEach(s => { if (s.ID == ID) { return Haversine(lonP, latP, s.longitude, s.latitude); });//returns in a string the distnace between the  station and given point                   
                }
            }
